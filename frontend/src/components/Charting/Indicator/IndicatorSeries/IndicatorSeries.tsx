    ...
    <React.Fragment>
      {
        Object.keys(charts.indicators).map((shortId: string) => {
          const indicatorParams: IndicatorObject = charts.indicators[shortId as keyof typeof charts.indicators]
          const indicatorData = indicatorParams.data;
          return indicatorData.map((d) => {
            return (
              <Line color={indicatorParams.color} key={`${props.id}-${shortId}`} id={props.id} indicatorId={shortId} data={d}/>
            )
          })
        })
      }
